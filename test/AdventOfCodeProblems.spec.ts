...
import { problem12a } from "../src/ProblemsAndInputs/problem12";
...
describe( 'Advent of Code Problems', () =>
{
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem2, [new Memory()] );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem3a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem3b );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem4a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem4b );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem5a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem5b );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem6a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem6b );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem7a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem7b );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem8a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem9a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem9b );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem10a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem10b );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    it( "Problem 11a - PainterRobot - Someone else's input data", async () =>
    {
        ...
        const result: ExecutionTimerResult = await measureExecutionTime( problem11a, [problemInput] );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    ...
    {
        const problemInput = readFileAsBigIntArray( './src/ProblemsAndInputs/problem11input.txt' );
        const result: ExecutionTimerResult = await measureExecutionTime( problem11b, [problemInput] );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
    } );

    it( 'Problem 12a - GravitySimulator', async () =>
    {
        const result: ExecutionTimerResult = await measureExecutionTime( problem12a );
        expect( result.runtimeSeconds ).to.be.lte( maxExpectedProblemRuntimeSeconds );
        expect( result.functionOutput ).to.equal( 8044 );
    } );
} );